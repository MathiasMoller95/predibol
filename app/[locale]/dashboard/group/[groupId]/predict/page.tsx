...
type LiveMatchRecord = MatchRecord & {
...
export type FinishedMatchPayload = MatchRecord & {
  home_score: number;
  away_score: number;
  predicted_home: number;
  predicted_away: number;
...
export type StickerRow = { team: string; tier: string };

const MATCH_SELECT_BASE =
  "id,phase,home_team,away_team,match_time,locked_at,status,home_win_odds,draw_odds,away_win_odds,ai_home_score,ai_away_score,knockout_label,advancing_team";
...
export default async function GroupPredictPage({ params }: Props) {
  ...
  const { data: group, error: groupError } = await supabase
    .from("groups")
    .select(
      "id,name,admin_id,logo_url,powers_double_down,powers_spy,powers_shield,points_correct_result,points_correct_difference,points_exact_score",
    )
    .eq("id", groupId)
    .single();
  ...
  const { data: allUserPredictions } = await supabase
    .from("predictions")
    .select("match_id,predicted_home,predicted_away,predicted_winner,predicted_advancing,points_earned")
    .eq("group_id", groupId)
    .eq("user_id", user.id);
  ...
  const { data: liveRows } = await supabase
    .from("matches")
    .select(`${MATCH_SELECT_BASE},home_score,away_score,advancing_team`)
    .eq("status", "live")
    .order("match_time", { ascending: true });

  const liveMatches = ((liveRows ?? []) as LiveMatchRecord[]).filter((m) => m.status === "live");

  const upcomingIds = upcomingMatches.map((m) => m.id);
  const liveIds = liveMatches.map((m) => m.id);

  let predictionsForUpcoming: PredictionRecord[] = [];
  if (upcomingIds.length > 0) {
    predictionsForUpcoming = allPredList.filter((p) => upcomingIds.includes(p.match_id));
  }

  const matchIdsForBadges = Array.from(new Set([...upcomingIds, ...liveIds]));

  let predictionsByMatch: Record<string, string[]> = {};
  if (matchIdsForBadges.length > 0) {
    const { data: allPreds } = await supabase
      .from("predictions")
      .select("match_id,user_id")
      .eq("group_id", groupId)
      .in("match_id", matchIdsForBadges);
    ...
    finishedMatches = (finRows ?? [])
      .map((row) => {
        ...
        return {
          ...
          predicted_advancing: pr.predicted_advancing,
          points_earned: pr.points_earned ?? 0,
        } as FinishedMatchPayload;
      })
      .filter((x): x is FinishedMatchPayload => x != null);
  }

  const stickersByMatch: Record<string, StickerRow[]> = {};
  ...
  if (finishedIds.length > 0) {
    ...
    const { data: stickerRows } = await (supabase as any)
      ...
      .eq("user_id", user.id)
      .eq("group_id", groupId)
      .in("earned_from_match_id", finishedIds);

    for (const row of (stickerRows ?? []) as {
      team: string;
      tier: string;
      earned_from_match_id: string | null;
    }[]) {
      const mid = row.earned_from_match_id;
      if (!mid) continue;
      (stickersByMatch[mid] ??= []).push({ team: row.team, tier: row.tier });
    }
  }
    ...
    <main className="animate-page-in min-h-screen bg-dark-900 px-4 py-8">
      <section className="mx-auto w-full max-w-4xl rounded-xl border border-dark-600 bg-dark-800 p-5 sm:p-6">
        ...
        <PredictForm
          ...
          initialPredictions={predictionsForUpcoming}
          ...
          copyPredictionOptions={copyOptions}
          copyPredictionTargetGroupId={groupId}
        />
      </section>
    </main>
  );
