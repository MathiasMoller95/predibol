  ...
  if (groups.length === 0) {
    return (
      <div className="mt-10 rounded-lg border border-slate-200 bg-slate-50 p-8 text-center">
        <p className="text-slate-700">{t("noGroups")}</p>
        <Link
          href={`/${locale}/dashboard/create-group`}
          className="mt-4 inline-flex rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-emerald-700"
        >
          {t("createGroup")}
        </Link>
      </div>
  ...
  return (
    <div className="mt-8">
      <input
        ...
        className="w-full max-w-md rounded-md border border-slate-300 px-3 py-2 text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200"
          ...
          const body = (
            <>
              ...
              <div className="pl-4">
                ...
                {group.description ? (
                  <p className="mt-2 line-clamp-3 text-sm text-slate-600">{group.description}</p>
                ) : null}
                <p className="mt-3 text-sm text-slate-500">👥 {t("memberCount", { count: group.memberCount })}</p>
                {err ? <p className="mt-2 text-sm text-red-600">{err}</p> : null}
                {justJoinedId === group.id ? (
                  <p className="mt-2 text-sm font-medium text-emerald-700">{t("joined")}</p>
                ) : null}
                ...
                {!group.isMember ? (
                  <button
                    ...
                    className="mt-4 w-full rounded-md bg-emerald-600 px-3 py-2 text-sm font-medium text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
                  >
                    {t("join")}
                  </button>
                ) : null}
          ...
          if (group.isMember) {
            return (
              <Link
                ...
                className="relative block overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-slate-300 hover:shadow-md"
              >
                {body}
              </Link>
            );
      ...
      {filtered.length === 0 ? (
        <p className="mt-8 text-center text-sm text-slate-600">{t("noSearchResults")}</p>
      ) : null}
    </div>
  );
}
